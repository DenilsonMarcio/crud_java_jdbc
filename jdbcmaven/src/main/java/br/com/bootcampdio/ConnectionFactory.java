...
public class ConnectionFactory {
  ...
  // Construtor declarado como privado. Evitando assim criar instâncias da
  // fábrica.
  ...
  public static Connection getConnection() {
    ...
    // 1 - Declarar objeto conexão (irá receber uma conexão após executar os passos
    // abaixo)
    Connection connection = null;

    // 2 - Carregar arquivo de propriedades para pegar parâmetros necessários para
    ...
    try (InputStream input = ConnectionFactory.class.getClassLoader().getResourceAsStream("connection.properties")) {
      ...
      // 5 - Criar conexão usando o DriverManager, passando como parâmetros a string
